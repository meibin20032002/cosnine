...
<?php
		if($this->manage) {
?>
					<a href="<?php echo hikashop_completeLink('cart&task=edit&cart_type='.$this->cart_type.'&cid[]='.$cart->cart_id);?>">
						<img src="<?php echo HIKASHOP_IMAGES; ?>edit.png"/>
					</a>
					<a href="<?php echo hikashop_completeLink('cart&task=edit&cart_type='.$this->cart_type.'&cart_id='.$cart->cart_id.'&cid[]='.$cart->cart_id,false,true); ?>"><?php
...
<?php
		...
		if(empty($cart->user) || $cart->user_id == 0) {
			...
			if($this->manageUser)
				echo $cart->user->user_email.'<a href="'.$url.$target.'"><img src="'.HIKASHOP_IMAGES.'edit.png" alt="edit"/></a>';
		}
				?></td>
				<td class="hk_center"><?php
					echo $cart->cart_ip;
				...
				<td style="text-align: center;"><?php
					if(!empty($cart->cart_current)) {
						echo '<img src="'.HIKASHOP_IMAGES.'icons/icon-16-publish.png" alt="'.JText::_('HIKASHOP_YES').'"/>';
					} else {
						echo '<img src="'.HIKASHOP_IMAGES.'icons/icon-16-unpublish.png" alt="'.JText::_('HIKASHOP_NO').'"/>';
					}
				?></td>
				<td class="hk_center"><?php
