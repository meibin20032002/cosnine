...
?>				<?php
				...
				<table class="paramlist admintable table">
					<tr id="applyOntext" <?php echo $dipslayApplyText; ?>>
						...
						<td >
							<?php echo $this->orderType->display('data[filter][filter_data_text][]',@$this->element->filter_data, 'product_filter', 'class="inputbox" size="8" multiple="multiple"', false); ?>
						</td>
					</tr>
					...
					<tr id="searchProcessing" <?php echo $searchProcessing; ?>>
						...
						<td>
							<?php
								...
								echo JHTML::_('hikaselect.genericlist', $arr, "data[filter][searchProcessing]", 'class="inputbox" size="1"', 'value', 'text', @$this->element->filter_options['searchProcessing']);
							 ?>
						</td>
					</tr>
					<tr id="logic" <?php echo $logic; ?>>
						...
						<td><?php
							...
							echo JHTML::_('hikaselect.genericlist', $arr, "data[filter][logic]", 'class="inputbox" size="1"', 'value', 'text', @$this->element->filter_options['logic']);
						?></td>
					</tr>
					<tr id="max_char">
						<td class="key">
					...
					<tr id="product_information">
						...
						<td>
							<?php
								echo $this->product_informationType->display("data[filter][product_information]", @$this->element->filter_options['product_information'],'', 'onchange="setVisibleUnit(this.value);"', 'product_information_value');
							?>
						</td>
					<tr/>
					...
					<tr id="weight_unit" <?php echo $weightUnitDisplay; ?>>
						...
						<td>
							<?php
								echo $this->weight->display('data[filter][weight_unit]',@$this->element->filter_options['information_unit']);
							?>
						</td>
					<tr/>
					...
					<tr id="cursorNumber" <?php echo $displayCursors; ?>>
						...
						<td>
							<select name="data[filter][cursor_number]">
								...
								$maxColumn=5;
								for($i=1;$i<$maxColumn;$i++){  ?>
									<option <?php if(@$this->element->filter_options['cursor_number'] == $i) echo "selected=\"selected\""; ?> value='<?php echo $i; ?>' ><?php echo $i; ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
