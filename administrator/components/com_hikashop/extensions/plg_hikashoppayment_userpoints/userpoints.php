...
class plgHikashoppaymentUserpoints extends hikashopPaymentPlugin {
	...
	public function checkPaymentDisplay(&$method, &$order) {
		...
		if(!empty($method->payment_params->virtual_coupon)) {
			return $this->checkPaymentDisplay_VirtualCoupon($method, $order);
		}
		return $this->checkPaymentDisplay_ClassicalMethod($method, $order);
	}

	protected function checkPaymentDisplay_VirtualCoupon(&$method, &$order) {
