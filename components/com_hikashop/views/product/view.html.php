	...
	public function filter() {
		...
		if(!empty($this->params) && $this->params->get('module') == 'mod_hikashop_filter'){
			...
			if(!$this->params->get('force_redirect',0)){
				if(hikaInput::get()->getVar('option','')=='com_hikashop'){
					$cid = hikaInput::get()->getInt('cid');
					if($cid){
						if(hikaInput::get()->getVar('ctrl','product')!='product'){
							if(hikaInput::get()->getVar('ctrl','product')!='category' || hikaInput::get()->getVar('task','listing')!='listing'){
								$cid = 0;
							}
						}elseif(hikaInput::get()->getVar('task','listing')!='listing'){
							$cid = 0;
						}
					}elseif(in_array(hikaInput::get()->getVar('ctrl','product'),array('product','category'))&& hikaInput::get()->getVar('task','listing')=='listing'){
						global $Itemid;
